...
fn generated_game_ids_are_unique_for_database_rows() {
    ...
    assert_eq!(
        games.len(),
        game_data::game_database_row_count(),
...
fn game_database_console_ids_have_console_metadata() {
    ...
    for console_id in game_data::game_database_console_ids() {
        assert!(
...
fn filename_aliases_match_saved_state_console_ids() {
    assert_eq!(
        game_data::map_filename_to_console_id("playstationvita"),
        Some("psvita")
    );
    assert_eq!(
        game_data::map_filename_to_console_id("nintendods"),
        Some("ds")
    );
    assert_eq!(
        game_data::map_filename_to_console_id("gameboyadvance"),
        Some("gba")
    );
}
...
fn console_id_extraction_allows_hyphenated_console_ids() {
    assert_eq!(
        game_data::get_console_from_id("pc-engine-0123456789abcdef"),
        "pc-engine"
    );
    assert_eq!(
        game_data::get_console_from_id("psvita-0123456789abcdef"),
        "psvita"
    );
    assert_eq!(game_data::get_console_from_id("malformed"), "");
}

#[test]
fn export_json_has_stable_ordering() {
    let mut app = MemoryPakApp {
    ...
    let first = persistence::export_json(&app).expect("export should serialize");
    app.console_states.insert(
        "gamecube".to_string(),
        ConsoleState {
            console_id: "gamecube".to_string(),
            favorite: true,
            ..Default::default()
        },
    );
    app.console_states.remove("gamecube");
    let second = persistence::export_json(&app).expect("export should serialize");
