...
pub fn run_memory_pak_native(
    ...
    let options = {
        ...
        options.android_app = android_app;
        options
    };
...
#[cfg(target_os = "android")]
fn load_app_icon() -> Option<egui::IconData> {
    // No embedded window icon is needed for Android.
    None
}
...
impl MemoryPakApp {
    ...
    fn maybe_flush_saves(&mut self) {
        ...
        if !self.pending_console_save
            && !self.pending_game_save
            && !self.pending_lego_save
            && !self.pending_skylanders_save
        {
        ...
        if show_sidebar {
            egui::SidePanel::left("sidebar")
                ...
                .show(ctx, |ui| {
                    ...
                    if ui
                        .selectable_label(self.ui_state.active_tab == Tab::Skylanders, "Skylanders")
                        .clicked()
                    {
